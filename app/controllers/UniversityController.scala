...
class UniversityController @Inject()(
  ...
  def getById(id: Int): Action[AnyContent] = Action.async { implicit request: Request[AnyContent] =>
  ...
  def searchUniversityByName(name: String): Action[AnyContent] = Action.async { implicit request: Request[AnyContent] =>
    universityRepository.searchUniversityByName(name).map {
      university => Ok(Json.toJson(university)).withHeaders("Access-Control-Allow-Origin" -> "*")
    }
  }

  def universityWithStudentCount():Action[AnyContent] = Action.async{ implicit request : Request[AnyContent] =>{
